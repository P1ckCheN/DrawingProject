...
int ParsingDataCache(ColorParm in_color, LinewidthParm in_linewidth, 
                     int* out_color, int& out_linewidth, 
...
int ReadXmlFile(DrawingBoard* board)
{
  ...
  board->color_linewidth_database_.state_ = DataState::NO_UPDATE;
  ...
  TiXmlElement* xml_color_configure = 
                xml_configure->FirstChildElement("color_configure");
  TiXmlElement* xml_linewidth_configure = 
                xml_color_configure->NextSiblingElement("linewidth_configure");
  ...
  while (xml_color != nullptr) {
    ...
    color_value_string = 
      xml_color_name->NextSiblingElement("value")->GetText();
    Transform(color_value_string, color_transform);
    board->color_linewidth_database_.color_cache_[s] = new int[3];
    for (int i = 0; i < 3; i++) {
      board->color_linewidth_database_.color_cache_[s][i] = color_transform[i];
    }
    xml_color = xml_color->NextSiblingElement();
  }

  TiXmlElement* xml_linewidth = 
                xml_linewidth_configure->FirstChildElement("linewidth");
  while (xml_linewidth != nullptr) {
    TiXmlElement* xml_linewidth_name = 
                  xml_linewidth->FirstChildElement("name");
    std::string s = xml_linewidth_name->GetText();
    board->color_linewidth_database_.linewidth_cache_[s] = 
      std::stoi(xml_linewidth_name->NextSiblingElement("value")->GetText());
    xml_linewidth = xml_linewidth->NextSiblingElement();
  }

  if (board->color_linewidth_database_.color_cache_.empty()) {
    return ERROR_EMPTY_COLOR;
  }
  if (board->color_linewidth_database_.linewidth_cache_.empty()) {
    return ERROR_EMPTY_LINEWIDTH;
  }

  board->color_linewidth_database_.state_ = DataState::UPDATEED;
  board->color_linewidth_cache_.state_ = DataState::DELETED;
  board->color_linewidth_cache_.deletecache();
  board->SetRead(true);
  return SUCCESS;
}